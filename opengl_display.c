...
static void poll_logger(void)
{
    ...
    if (strncmp(latest, seen, LOG_WIDTH - 1) == 0)
        return;
...
void opengl_client_set_progress(int pct)
{
    ...
    s_progress = (pct < 0) ? 0 : (pct > 100) ? 100
                                             : pct;
...
static void display_cb(void)
{
    ...
    int total = logger_get_total_connections();
    char last_ip[INET_ADDRSTRLEN], last_action[128];
    logger_get_last_client(last_ip, sizeof(last_ip),
                           last_action, sizeof(last_action));
    ...
    glColor3f(TEXT_R, TEXT_G, TEXT_B);
    char badge[48];
    snprintf(badge, sizeof(badge), "  Live: %d  Total: %d  ", cc, total);
    draw_string(fw - 172, fh - 28, badge, GLUT_BITMAP_HELVETICA_12);

    /* last client info line below title */
    glColor3f(SUB_R, SUB_G, SUB_B);
    char last_info[160];
    snprintf(last_info, sizeof(last_info),
             "Last: %s  —  %s", last_ip, last_action);
    draw_string(14, fh - 52, last_info, GLUT_BITMAP_8_BY_13);
